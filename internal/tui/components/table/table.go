...
// ReplaceRows replaces the rows and moves to the given row, keeping it where
// the current row is shown, so the selection doesn't jump when rows are
// added or removed above it.
func (m *Model) ReplaceRows(rows []Row, currItem int) {
	m.Rows = rows
	m.rowsViewport.ReplaceItems(len(m.Rows), currItem)
	m.SyncViewPortContent()
	m.rowsViewport.ScrollToTopBound()
}
...
func (m *Model) renderBody() string {
	...
	return zone.Mark(rowsZoneId, m.rowsViewport.View())
}

// rowsZoneId marks the table's rows. Only one table is shown at a time.
const rowsZoneId = "table-rows"

// ItemAt returns the row under the mouse, or -1 for none.
func (m *Model) ItemAt(msg tea.MouseMsg) int {
	z := zone.Get(rowsZoneId)
	if m.isLoading || !z.InBounds(msg) {
		return -1
	}
	return m.rowsViewport.ItemAtLine(msg.Mouse().Y - z.StartY)
}

// SelectItem moves to the given row.
func (m *Model) SelectItem(item int) int {
	currItem := m.rowsViewport.SelectItem(item)
	m.SyncViewPortContent()
