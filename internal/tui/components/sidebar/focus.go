...
// FocusLine focuses the item on the given content line, e.g. one that was
// clicked, without scrolling. It reports whether there was one to focus.
func (m *Model) FocusLine(line int) bool {
	items := m.focusItems()
	if len(items) == 0 || line < 0 || line >= m.viewport.TotalLineCount() {
		return false
	}
	i := itemAt(items, line)
	if items[i].preamble {
